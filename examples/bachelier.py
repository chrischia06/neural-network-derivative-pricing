...
def bachelier_eval_wrapper(X_df: pd.DataFrame, 
                    true_val:np.array, 
                    preds:np.array,
                    grads:np.array,
    ...
    sns.scatterplot(X_df['basket'], X_df['call_payoff'], label="Sample Sample Payoffs", ax = ax[0])
    ...
    sns.scatterplot(X_df['basket'], np.maximum(X_df['basket'] - 1.0, 0), label="Lower Bound", ax = ax[0])
    ax[0].legend()
    ...
    pred_stats = pd.DataFrame([diagnosis_pred(X_df['call_analytic'].values, preds, lower_bound, upper_bound)], 
...
#     pde_stats = pd.DataFrame(diagnosis_pde(PDE_err), index = [METHOD]).add_prefix("PDE_")
    ...
    grad_stats = pd.DataFrame(diagnosis_pred(true_factor, grads, lower_bound = 0), index=[METHOD]).add_prefix("grad_")
    """
    Display Statistics
    """
