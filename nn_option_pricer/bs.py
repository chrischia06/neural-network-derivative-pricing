...
def bs_eval_wrapper(X_df: pd.DataFrame, 
    ...
    try:
        plot_preds(moneyness = moneyness, 
                   ...
                   preds = preds,
                   method = METHOD)
        pred_stats = diagnosis_pred(true_val, preds, lower_bound, upper_bound, METHOD).add_prefix("pred_")
    ...
    try:
        ...
        ax2[1].scatter(X_df["log(S/K)"], true_second_order - hessian_moneyness[:, f_to_i("log(S/K)")])
...
def make_GBM_dataset(param_space:dict, n_samples:int, n_times:int, T:float, seed:int = 42) -> pd.DataFrame:
    """
    ...
    SKs = rng.uniform(np.log(param_space['log(S/K)'][0]), 
                      np.log(param_space['log(S/K)'][1]), n_samples) # log-moneyness
    sigma = rng.uniform(param_space['sigma'][0], param_space['sigma'][1], n_samples) # volatilty
    # state space
