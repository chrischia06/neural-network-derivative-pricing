...
def cev_step(F0:float, V0:float, beta:float, dt:float, Z1:float, rho:float, Z2:float):
    ...
    """
    return jnp.abs(F0 + V0 * (F0 ** (beta)) * (jnp.sqrt(dt) * (rho * Z2 + jnp.sqrt(1 - rho ** 2) * Z1)))
...
def sabr_expansion(F, K, vol, beta, rho, vol_of_vol, ttm):
    ...
    denom = (F_K ** 0.5) * (1 + ((1 - beta) ** 2) / 24 *  mon ** 2 + ((1 - beta) ** 4) / 1920 * mon ** 4)

 
    return ((F == K) * vol * num / denom) + (np.nan_to_num(z / x) * vol * num / denom)

def sabr_pde_err(X_df:pd.DataFrame,
                 grads:np.array,
                 hessian_moneyness:np.array,
                 hessian_vol:np.array,
                 f_to_i:Callable) -> np.array:
    """
...
def sabr_eval_wrapper(X_df: pd.DataFrame, 
                    true_val:np.array, 
                    preds:np.array,
                    grads:np.array,
                    hessian_moneyness:np.array,
