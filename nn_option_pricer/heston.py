...
def heston_cdf(x, tau, v, kappa, vbar, vol_of_vol, rho):
    ...
    return (
        ...
        + (1 / np.pi)
        * quad(
            lambda u: np.real(np.exp(c_func(u) + 1j * u * x) / (u * 1j)),
            0,
            100,
            limit=1000,
        )[0]
...
def density2(x, tau, v, kappa, vbar, vol_of_vol, rho, eps=1e-6):
    ...
    """
    return (
        f(x + eps, tau, v, kappa, vbar, vol_of_vol, rho)
        - f(x, tau, v, kappa, vbar, vol_of_vol, rho)
    ) / eps
...
def heston_call_quad(k, tau, v, kappa, vbar, vol_of_vol, rho):
    ...
    return quad(lambda y: np.maximum((np.exp(y) - 1.0), 0) * func(y), k, 100, limit=30)[
        0
    ]
